                ...
                <div class="skill-level-section">
                  <div class="level-display">
                    ...
                    <el-rate
                      ...
                      :max="5"
                      show-score
                      score-template="等级 {value}"
                      @change="handleLevelChange(skill)"
                    />
                  </div>
      ...
      >
        ...
        <el-form-item label="技能等级" prop="skillLevel">
          <el-rate
            ...
            :max="5"
            show-text
            :texts="['了解', '熟悉', '熟练', '精通', '专家']"
          />
        </el-form-item>

        <el-form-item label="学习时长" prop="learningTime">
          <el-input-number
            v-model="skillForm.learningTime"
            :min="0"
            :step="10"
            placeholder="小时"
            style="width: 100%"
        ...
        <el-form-item label="相关项目" prop="projectCount">
          <el-input-number
            v-model="skillForm.projectCount"
            :min="0"
            placeholder="项目数量"
            style="width: 100%"
          />
...
import {
  Histogram, TrendCharts, InfoFilled, Search, Plus, Upload,
  List, Edit, Delete, Medal, Star
...
const skillStats = computed(() => {
  ...
  const average = total > 0 
    ? (skillList.value.reduce((sum, skill) => sum + skill.skillLevel, 0) / total).toFixed(1)
    : 0
...
const updateRadarChart = (chart?: any) => {
  ...
  const categoryStats = skillCategories.slice(1).map(category => {
    ...
    const averageLevel = categorySkills.length > 0
      ? categorySkills.reduce((sum, skill) => sum + skill.skillLevel, 0) / categorySkills.length
      : 0
        ...
        .level-description {
          .level-item {
            ...
            .level-badge {
              ...
              &.level-1 { background-color: #f56c6c; }
              &.level-2 { background-color: #e6a23c; }
              &.level-3 { background-color: #409eff; }
              &.level-4 { background-color: #67c23a; }
              &.level-5 { background-color: #005BAC; }
            }
